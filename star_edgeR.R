...
#of samples (24 samples in this case because the smallest group size is two). The function
#accesses the group factor contained in d in order to compute the minimum group size, but
...
write.table(as.data.frame(sig_pvalue),"star_edgeR.csv", row.names = T,sep=",",col.names=T)
